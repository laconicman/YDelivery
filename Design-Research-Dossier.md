...
### 3.2 Pasted links & raw coordinates — the grammar table

The paste affordance: when the pasteboard (or a typed/pasted string in the search field)
matches any grammar below, show a one-tap suggestion row — "Похоже на ссылку с точкой —
использовать её?" ("Looks like a map link — use this point?") — with the resolved
coordinates/label, plus a decline. Failure copy must distinguish "not a map link" (silent —
no affordance shown) from "map link we couldn't resolve" (rendered state: "Ссылка есть, а
точки в ней нет — откройте её и скопируйте адрес" + the raw URL kept as text). Note iOS
pasteboard-privacy: reading the pasteboard uninvited triggers the system banner — prefer
`UIPasteboard.detectPatterns`/paste-button entry points *(implementation note for the
register, affects the affordance's shape: it is a button, not an ambient scan)*.

**What the design may promise, per provider** (parse rules the implementation will honor):
...
| Google Maps, path view | `https://www.google.com/maps/@55.7558,37.6173,15z` | `@lat,lng,zoom` = **map center**, not the pin — use only as fallback | ✅ |
...
| **geo: URI (RFC 5870)** | `geo:55.7558,37.6173`, `geo:13.4125,103.8667,14` | **lat,lng[,alt]**, WGS-84 default; params like `;u=` (uncertainty) may follow; Android extension `geo:0,0?q=lat,lng(label)` | ✅ |
...
| Apple Maps, unified (iOS 18.4+/web) | `https://maps.apple.com/place?coordinate=40.779092,-73.962932&name=…` | `coordinate` = **lat,lng**; `place-id` forms have no coords | ✅ (`coordinate`) |
...
Sources: Yandex launch-URL docs (https://yandex.com/dev/yandex-apps-launch-maps/doc/en/concepts/yandexmaps-web),
Yandex short-link expansion example (https://community.glideapps.com/t/expand-short-link-to-glide/66837),
Google Maps URLs (https://developers.google.com/maps/documentation/urls/get-started),
Google link-parsing taxonomy incl. `@` vs `!3d!4d` and short-link unfurling
(https://github.com/kdcokenny/google-maps-link-parser,
...
**The trap to design for: coordinate order flips per provider.** Yandex/2GIS web params are
lon,lat; Yandex `rtext`, geo:, Apple, Google are lat,lng. West-of-Urals Russia both values
are ≤ 90, so magnitude cannot disambiguate — the parser must key on host+parameter, never
guess. The *design* consequence: the paste affordance always shows the resolved point as a
mini-map/address for human verification before it is committed to the route, because a
silent lon/lat swap puts the pin in the Barents Sea.

**Short links are the only network-dependent class** (`yandex.ru/maps/-/…`,
`maps.app.goo.gl`, `goo.gl/maps`, `go.2gis.com`): design a brief "Разворачиваем ссылку…"
(expanding…) state on the suggestion row with offline fallback copy ("Не получилось
развернуть короткую ссылку без сети").
