...
extension UUID {
    ...
    nonisolated static func derived(namespace: UUID, _ components: String...) -> UUID {
    ...
    nonisolated enum DerivedNamespace {
        /// Migrated/recorded route children: `orderID ‖ kind ‖ index`.
        static let orderChild = UUID(uuidString: "7D1E0A3E-5B4C-4A2F-9C8D-1E2F3A4B5C6D")!
        /// Discovered orders: `providerAccountRef ‖ claimID`.
        static let discoveredOrder = UUID(uuidString: "8E2F1B4F-6C5D-4B3A-AD9E-2F3A4B5C6D7E")!
        /// Provider events: `orderID ‖ providerEventID` or `orderID ‖ status ‖ source`.
        static let providerEvent = UUID(uuidString: "9F3A2C5A-7D6E-4C4B-BEAF-3A4B5C6D7E8F")!
        /// The discovery-retry queue: `providerAccountRef ‖ claimID`.
        static let pendingDiscovery = UUID(uuidString: "AF4B3D6B-8E7F-4D5C-CFB0-4B5C6D7E8F9A")!
    }
}
